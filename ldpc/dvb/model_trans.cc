...
int main(int argc, char **argv)
{
	...
	table_model << "	(and (bvuge (bvsub a b) d) (bvuge (bvsub (_ bv" << ptys.size()-1 << " " << vlen << ") (bvsub a b)) (bvsub c (_ bv1 " << vlen << "))))" << std::endl;
	...
	for (int pty0 = 0; pty0 < ptys.size(); ++pty0) {
		for (int pty1 = pty0+1; pty1 < ptys.size(); ++pty1) {
			...
			table_model << "	(delay P" << pty0 << " P" << pty1 << " (_ bv" << delay0 << " " << vlen << ") (_ bv" << delay1 << " " << vlen << "))" << std::endl;
		}
	}
	table_model << "))" << std::endl;
